...
iPhone / iPad など RAM の少ない端末の Safari では、長尺・高解像度の動画を
変換しようとするとブラウザのメモリ上限（端末・OS 依存。iPhone 8 等では数百 MB
程度）を超え、**タブが強制的に再読み込みされる**ことがあります。再読み込み
直前に「予期せぬエラーが発生したため…」「問題が発生したため…」のようなバナーが
一瞬表示される場合がありますが、これは本アプリのエラーではなく Safari 側の
WebContent プロセスが OOM で再起動した際の挙動です。

本アプリは純粋な静的 SPA として動画をブラウザ内のメモリに保持するため、これは
仕様上のトレードオフです（サーバへの送信なしを最優先）。モバイルや旧端末では
**数十秒程度の短い動画から試す**ことを推奨します。PC など RAM に余裕のある環境
では 500 MB 程度を目安に扱えます。

実機確認状況（2026-05-15 時点）:
